...
func (s *Runner) Run(ctx context.Context, containerName, waitMessage string, args ...string) error {
	...
	spin := spinner.New(ctx, "container "+containerName)
	spin.Message("starting")
	w := s.start(procCtx, containerName, envFile, args)
	if w.err == nil {
	...
	if !ud.Containerized() {
		...
		if m := s.Mount; m != nil {
			...
			if err != nil {
				...
				w.volumes, w.err = docker.StartVolumeMounts(ctx, pluginName, daemonName, container, m.Port, m.Mounts, nil)
				if w.err != nil {
