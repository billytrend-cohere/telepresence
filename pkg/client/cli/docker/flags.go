...
// when starting it.
func (f *Flags) PullOrBuildImage(ctx context.Context, defaultContainerName string) ([]string, error) {
	if f.Image != "" {
		return f.args, docker.PullImage(ctx, f.Image)
	}
	...
	if err != nil {
		return f.args, spin.Error(err)
	}
	spin.DoneMsg("image built successfully")
	if f.imageIndex < 0 {
		return []string{"--name", defaultContainerName, imageID}, nil
	}

	args := slices.Clone(f.args)
	args[f.imageIndex] = imageID
	name, err := flags.GetUnparsedValue(f.args, "--name")
	if err != nil {
		return args, err
	}
	if name == "" {
		args = append([]string{"--name", defaultContainerName}, args...)
	}
	return args, nil
}
