...
func (s *session) watchAgentsLoop(ctx context.Context) error {
	...
	for ctx.Err() == nil {
		...
		s.handleAgentSnapshot(snapshot.Agents)
	}
	return nil
}

func (s *session) handleAgentSnapshot(infos []*manager.AgentInfo) {
	...
	s.ingestTracker.cancelUnwanted()
}
